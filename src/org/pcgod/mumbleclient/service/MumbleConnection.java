...
public class MumbleConnection implements Runnable {
     ...
     *
     * @param connectionHost Host interface for this Connection
     * @param host           Mumble server host address
     ...
     * @param b Protocol Buffer message builder
     */
     ...
     *
     * @param buffer   Udp message buffer
     * @param length   Message length
     * @param forceUdp True if the message should never be tunneled through TCP
     */
...
//            }
    ...
    protected Socket connectTcp(byte[] certificate, char[] certificatePassword) {
        ...
        try {
            ...
            /* connect */
            SSLSocket sock = (SSLSocket) cntxt.getSocketFactory().createSocket(hostAddress, port);
            sock.setUseClientMode(true);
            sock.setEnabledProtocols(new String[] {"TLSv1"});
            sock.startHandshake();

            Log.d(TAG, "TCP/SSL socket opened [secure]");
